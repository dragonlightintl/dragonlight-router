...
class OpenAIBackend(GenerativeBackend):
    ...
    async def generate(
        ...
        *,
        ...
        """Generate text using OpenAI's API (mocked for development)."""
        # Mock implementation: yield a mock response without making actual API calls
        if not self._config.env_key:
            yield f"[Mock OpenAI] Error: API key not configured for backend {self._config.name}"
            return

        api_key = os.getenv(self._config.env_key)
        if not api_key:
            yield f"[Mock OpenAI] Error: Environment variable {self._config.env_key} not set"
            return
        ...
        mock_response = f"[Mock OpenAI {self._config.model}] Response to: {user_message[:50]}..."
        yield mock_response

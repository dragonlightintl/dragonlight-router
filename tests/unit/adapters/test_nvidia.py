"""Tests for the NVIDIA NIM backend adapter.
...
def _make_nvidia_backend(
    make_backend_config,
    transport: httpx.MockTransport | None = None,
    *,
    env_key: str = "NVIDIA_API_KEY",
    env_value: str | None = "nvapi-test-key",
) -> NvidiaBackend:
    """Build a NvidiaBackend with optional transport injection."""
    ...
    with patch.dict("os.environ", env, clear=True):
        return NvidiaBackend(config, _transport=transport)
...
def backend(make_backend_config):
    """NvidiaBackend wired to a fake API key via env (no transport)."""
    config = make_backend_config(
        name="nim-test",
        provider="nvidia",
        model="nvidia/llama-3.1-nemotron-70b-instruct",
    ...
    with patch.dict("os.environ", {"NVIDIA_API_KEY": "nvapi-test-key"}):
        yield NvidiaBackend(config)


@pytest.fixture
def backend_no_key(make_backend_config):
    """NvidiaBackend with no API key configured."""
    config = make_backend_config(
        name="nim-test",
        provider="nvidia",
        model="nvidia/llama-3.1-nemotron-70b-instruct",
...
async def test_generate_non_streaming(make_backend_config):
    ...
    assert chunks == ["Hello from NVIDIA NIM"]
...
def test_default_base_url():
    """The default NVIDIA NIM base URL is correct."""
    assert _DEFAULT_BASE_URL == "https://integrate.api.nvidia.com"
...
async def test_auth_header_format(make_backend_config):
    ...
    assert chunks == ["ok"]
    assert captured_request[0].headers["authorization"] == "Bearer nvapi-test-key"
...
def test_properties(backend):
    ...
    assert backend.config.name == "nim-test"
